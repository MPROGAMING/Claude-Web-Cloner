...
// ── Fork reason ────────────────────────────────────────────────────────────
// This repo is a Next.js application, not a published DS package: there is no
// dist/, so the entry is synthesized by walking srcDir — and srcDir has to be
// src/components for group derivation to work. But src/components/{app,auth,
// workspace} import server actions and Supabase, which transitively import
// "server-only" — a module whose browser build throws at import time. One of
// those in the synthesized entry throws before any component mounts, blanking
// every card in the bundle.
//
// No config key filters the walk (top-level config keys are validated strictly),
// so the scope is stated once in .design-sync/ds-scope.json and read by both
// this fork and .design-sync/prepare.mjs, which builds the matching entry.
const SCOPE_DIRS = JSON.parse(
  readFileSync(new URL('../ds-scope.json', import.meta.url), 'utf8'),
).dirs;
const inEntryScope = (p) => SCOPE_DIRS.some((d) => slash(p).includes(`/${d}/`));
...
// Fork: 'ui' dropped from the generic list — here src/components/ui IS the
// primitives dir, and treating it as generic dumps ~90 components into 'general'.
...
function deriveComponentsFromSrc(srcFiles) {
  ...
  // Fork: Map, not Set — the scan already knows which file each export came
  // from, and that is the only reliable way to place sub-exports. `CardHeader`
  // lives in card.tsx, so step 4's name-based fuzzy-find never finds it and it
  // loses both its group and its JSDoc. Upstream that is ~80 of 118 components
  // landing in 'general'.
  const seen = new Map();
  ...
  return [...seen.keys()].sort().map((name) => ({ name, group: 'general', srcPath: seen.get(name) }));
}
...
export async function resolvePackage(ctx) {
  ...
  // Fork: scoped to the browser-safe dirs (see SCOPE_DIRS above). Scoping here
  // covers the synthesized entry, component discovery and src enrichment in one
  // place; cfg.componentSrcMap pins bypass this list, as upstream.
  const srcFiles = (srcRoot ? walk(srcRoot, (n) => /\.(tsx|jsx|mdx?)$/.test(n)) : []).filter(inEntryScope);
  ...
  // Fork: upstream only falls back to the src scan in synth-entry mode. This
  // repo passes --entry (a generated barrel — the only way to put PKG_DIR at the
  // repo root, see .design-sync/prepare.mjs), so synthEntry is false while there
  // is still no .d.ts tree to read exports from. Fall back whenever the .d.ts
  // scan came up empty, however the entry was resolved.
  ...
  if (srcRoot) {
    for (const c of components) {
      ...
      // Fork: c.srcPath from the src scan (change 2) is authoritative when set.
